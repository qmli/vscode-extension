...
export class InstantiationService implements IInstantiationService {
  ...
  private _createAndCacheServiceInstance<T>(id: ServiceIdentifier<T>, desc: SyncDescriptor<T>, _trace: Trace): T {
    ...
    let cycleCount = 0;
    const stack = [{ id: id, desc: desc, _trace: _trace }];
    while (stack.length) {
  ...
  private _createServiceInstance<T>(
    ...
    const idle = new IdleValue<any>(() => {
      ...
      for (const [key, values] of earlyListeners) {
        const candidate = (result as any)[key] as Event<any>;
    ...
    return new Proxy(Object.create(null), {
      ...
      set: function (_target: T, p: PropertyKey, value: any): boolean {
        idle.value[p] = value;
        return true;
      },
      getPrototypeOf: function (_target: T) {
        // eslint-disable-next-line @typescript-eslint/no-unsafe-return
