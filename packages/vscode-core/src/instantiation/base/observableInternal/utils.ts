...
import type { IObservable, IObserver, IReader, ITransaction } from './base';
import { BaseObservable, ConvenientObservable, getFunctionName, observableValue, transaction } from './base';
...
export function observableFromPromise<T>(promise: Promise<T>): IObservable<{ value?: T }> {
  ...
  promise.then((value) => {
    observable.set({ value: value }, undefined);
  });
  return observable;
}
